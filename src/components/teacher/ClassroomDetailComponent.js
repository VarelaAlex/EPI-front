...
let ClassroomDetail = (props) => {
	...
	return (
		<Spin spinning={ loading } tip="Loading" size="large">
			<Card
				title={ !editing ? <Space>{ classroomName } <EditOutlined onClick={ () => setEditing(true) }/></Space> : <><Row>
			...
			>
				...
				{ students.length <= 0 ? <Empty image={ Empty.PRESENTED_IMAGE_SIMPLE } description={ t("classrooms.detail.table.empty") }/> : <Table bordered columns={ columns }
				                                                                                                                                     dataSource={ students }/> }
