...
function deliverSnapshot(sceneId, canvas) {
	...
	let parametros = {
		"timezone": (
			            ...
			            -1
		            ), "experiment": idExperiment, "sceneId": sceneId, "canvas": canvas.toDataURL("image/png"), "timeStamp": Date.now(), "sessionId": user
	};
...
function createUser() {
	if ( localStorage.getItem("user") === null || localStorage.getItem("user") === undefined ) {
		...
		localStorage.setItem(
			"user",
			lettrs[ Math.floor(Math.random() * lettrs.length) ] + lettrs[ Math.floor(Math.random() * lettrs.length) ] + lettrs[ Math.floor(Math.random() * lettrs.length) ] + (
			Math.floor(Math.random() * (
				999999999999 - 100000000000
			)) + 100000000000
			                                                    ).toString() + Date.now().toString() + getDate()
		);
	}
...
function deliverChunk(chunk) {
	let parametros = {
		"timezone": (
			            ...
			            -1
		            ), "list": chunk, "idExperiment": idExperiment, "sessionId": user
	};
...
function postNumberDD(id, value) {
	let parametros = {
		"timezone": (
			            ...
			            -1
		            ), "id": id, "numberValue": value, "idExperiment": idExperiment, "sessionId": user
	};
...
function postStringDD(id, value) {
	let parametros = {
		"timezone": (
			            ...
			            -1
		            ), "id": id, "stringValue": value, "idExperiment": idExperiment, "sessionId": user
	};
	postAJAXDemographicData(parametros);
}
...
function postDateDD(id, value) {
	let parametros = {
		"timezone": (
			            ...
			            -1
		            ), "id": id, "dateValue": value, "idExperiment": idExperiment, "sessionId": user
	};
	postAJAXDemographicData(parametros);
}
...
export function registerValor2(value) {postNumberDD(20, value);}

export function registerValor3(value) {postDateDD(21, value);}